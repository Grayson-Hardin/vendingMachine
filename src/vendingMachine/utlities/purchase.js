...
function purchase(payment = [], itemSelection) {
    ...
    if (insufficientPayment(roundTheCoins, item) === true) {
        return { message: `Insufficient funds: item costs $${item.price}`, status: "error" };
    }
    if (paymentOverflow(roundTheCoins, item) === true) {
        const change = calculateChange(roundTheCoins, item);
        return { message: `${item.name} Purchased. Change returned: $${change}`, status: "success" };
    }
...
module.exports = { purchase, insufficientPayment, paymentOverflow, calculateChange };
